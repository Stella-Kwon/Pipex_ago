...
/*   Updated: 2023/03/13 13:31:21 by sukwon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (backup[fd] == NULL)
	{
		backup[fd] = (char *)malloc(1 * sizeof(char));
		backup[fd][0] = '\0';
	}
	backup[fd] = store_line(backup[fd], fd);
	line = seperate_line(backup[fd]);
	backup[fd] = store_rest(backup[fd]);
	return (line);
}
