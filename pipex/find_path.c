...
/*   Updated: 2023/03/11 19:23:20 by sukwon           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int count_argv(t_arg *stru, int argc, char **argv)
{
    ...
    {
        ft_strlcpy(stru->cmd[stru->j], argv[stru->i],ft_strlen(argv[stru->i] + 1));
...
int add_path(t_arg *stru, int argc, char **argv)
{
    ...
    while (stru->cmd[stru->j] != NULL)
    {
        ...
        free (stru->temp_path);//no use anymore so free it // strjoin has malloced value everytime its call, need to free
        ...
        if (access(stru->temp_wholepath, F_OK) == 0)
        {
            strlcpy(stru->cmd[stru->j],stru->temp_wholepath,ft_strlen(stru->temp_wholepath + 1));
...
/*https://velog.io/@kyj93790/Pipex-%EC%82%AC%EC%9A%A9%EA%B0%80%EB%8A%A5-%ED%95%A8%EC%88%98-*/
